 ...
 */
export async function createFeathersApp(httpServer, mongoURI) {
    ...
    app.configure(socketio((io) => {
        ...
        io.on('connection', (socket) => {
            ...
            const broadcastUserList = () => {
                const users = app
                    .channel('all')
                    .connections.map((c) => c.clientId);
                io.emit('user-list', { users });
            };
            broadcastUserList();
            io.emit('user-joined', { id: clientId, ts: Date.now() });
            // Ephemeral typing indicators — not persisted, not routed via channels
            socket.on('start-typing', () => socket.broadcast.emit('user-typing', { id: clientId }));
            socket.on('stop-typing', () => socket.broadcast.emit('user-stopped-typing', { id: clientId }));
