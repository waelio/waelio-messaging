...
interface WaelioMessageSendOptions { broadcast?: boolean }

type SendOnMode = 'connect' | 'manual' | 'click';

interface HubRegisterSuccess { type: 'register-success'; id: string }

// Light protocol subset for type safety
type InboundHubMessage =
    | HubRegisterSuccess
...
class WaelioMessageElement extends HTMLElement {
    ...
    set broadcast(v: boolean) { v ? this.setAttribute('broadcast', '') : this.removeAttribute('broadcast'); }

    get wsUrl() { return this.getAttribute('ws-url') || this._defaultWsUrl(); }
    set wsUrl(v: string) { this.setAttribute('ws-url', v); }

    get sendOn(): SendOnMode {
        const v = this.getAttribute('send-on');
        return (v === 'manual' || v === 'click') ? v : 'connect';
    }
    set sendOn(v: SendOnMode) { this.setAttribute('send-on', v); }

    get reconnect() { return this.hasAttribute('reconnect'); }
    set reconnect(v: boolean) { v ? this.setAttribute('reconnect', '') : this.removeAttribute('reconnect'); }

    connectedCallback() {
        if (!this.hasAttribute('reconnect')) this.reconnect = true;
    ...
    send(messageOverride?: string, targetOverride?: string, opts: WaelioMessageSendOptions = {}) {
        ...
        if (!payload) return; // Silently ignore empty
        if (!broadcast && !to) {
